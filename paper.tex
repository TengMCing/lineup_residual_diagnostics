...
Data collection resulted in a total of 7974 lineup evaluations, 3744 for
the non-linearity model, and 4230 for the heteroskedasticity model. This
was done in three collection periods, called I, II, III, with 2880, 2700
and 1674 lineups evaluated by 160, 150 and 133 participants,
respectively. There were 886 attention check lineups used to filter
false participant attempts. To estimate \(\alpha\) for calculating
statistical significance there were 720 evaluations of null lineups.
Neither the attention checks nor null lineups were used for the
subsequent analysis. The de-identified data, \texttt{vi\_survey}, is
made available in the R package, \texttt{visage}.

For clarity and consistency, the analysis in Sections
\ref{power-analysis}-\ref{hetero-analysis}, uses only on results from
lineups generated with uniform fitted values, a total of 4135 lineups
and XXX evaluations. More evaluations were made using the uniform fitted
value distribution, and it avoids potentially confounding results of the
analysis by combining values from different power curves. We also expect
that visual patterns are more clearly visible when the fitted value
distribution is uniform. The results in Section
\ref{effect-of-fitted-value-distributions} examine the effect of fitted
value distribution on power.
...
Figure \ref{fig:polypower} displays the estimated power of a visual
test, computed on evaluations of the lineups generated by the
non-linearity model with uniform fitted values. Power is plotted against
the natural logarithm of the effect size \(log_e(E)\), computed at a 5\%
significance level. The small plots at the bottom of this figure shows a
sequence of example residual plots of increasing effect size. These are
for the readers to compare what a residual plot might look like at
particular effect sizes.

As discussed in Section \ref{conventionally-testing-for-departures},
many conventional tests are available for detecting residual departures.
Implementation-wise, the built-in R package \texttt{stats} provides some
commonly used residual-based tests, such as Shapiro-Wilk test. A more
comprehensive collection of regression diagnostics tests can be found in
the R package \texttt{lmtest} \citep{lmtest}. In terms of
heteroskedasticity diagnostics, the R package \texttt{skedastic}
