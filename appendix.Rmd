...
Throughout the study, we have collected `r vi_survey %>% filter(!attention_check, !null_lineup) %>% nrow()` evaluations on `r vi_survey %>% filter(!attention_check, !null_lineup) %>% count(unique_lineup_id) %>% nrow()` non-null lineups. Table \ref{tab:count-lineup} gives further details about the number of evaluations and lineups over departure types and data collection periods. 
...
matrix(c(c(576, 0, 144, 0, 540, 135, 1116),
       ...
       byrow = TRUE) %>%
  data.frame() %>%
  add_column(name = c("Lineups", "Evaluations"), .before = 1) %>%
  kableExtra::kable(format = "latex", 
                    booktabs = TRUE,
                    label = "count-lineup",
                    caption = "Count of lineups and evaluations over departure types and data collection periods.",
