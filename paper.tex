...
Table \ref{tab:example-residual-plot-table} shows the RESET,
Breusch-Pagan and Shapiro-Wilk test results (\(p\)-values) for the
residual plots shown in Figure
\ref{fig:residual-plot-common-departures}. The RESET test and
Breusch-Pagan test were computed using the \texttt{resettest} and
\texttt{bptest} functions from R package \texttt{lmtest}, respectively.
The Shapiro-Wilk test was computed using the \texttt{shapiro.test} from
the core R package \texttt{stats}. (The the R package \texttt{skedastic}
\citep{skedastic} contains a large collection of tests for
heteroskedasticity.) The RESET test is exact, but the power parameter
must be chosen. \citet{ramsey_tests_1969} suggests using a power of
four, which is what we used. The Breusch-Pagan and Shapiro-Wilk tests
are approximate.
...
\citep{cook_applied_1999}. With a sufficiently large sample size,
residual-based tests may reject when there is only a slight departure.
This is a situation that is addressed by the results in this paper
because from a practical perspective we would only want to flag the
model as problematic when ther is a XXX diagnostic practical
significance. XXX check heteroscedastic spelling
