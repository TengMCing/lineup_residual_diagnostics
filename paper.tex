...
crowd-sourcing platform \citep{palan2018prolific} is presented with a
block of 20 lineups. A lineup consists of a randomly placed data plot
and 19 null plots, which are all residual plots drawn with raw residuals
on the y-axis and fitted values on the x-axis. An additional horizontal
red line is added at \(y = 0\) as a visual reference. The data in the
data plot is simulated from one of two models described in Section
...
by 443 participants throughout three data collection periods. Details
about lineup allocation and data collection are provided in the
Appendix.
...
\(\boldsymbol{\varepsilon}\), \(\boldsymbol{z}\) are vectors of size
\(n\), \(\boldsymbol{1}_n\) is a vector of ones of size \(n\),
\(\boldsymbol{z} \propto He_j(\boldsymbol{x})\), \(He_{j}(.)\) is the
\(j\)th-order probabilist's Hermite polynomials
\citetext{\citealp{hermite1864nouveau}; \citealp[originally
by][]{de1820theorie}}.

To fit a misspecified model, the \(\boldsymbol{z}\) term, which is a
higher order term of \(\boldsymbol{x}\), was omitted from the null
regression formula
...
Four different values of \(j = 2, 3, 6, 18\) were chosen so that
distinct shapes of non-linearity were included in the residual plot.
These include ``U'', ``S'', ``M'' and ``triple-U'' shape as shown in
Figure \ref{fig:different-shape-of-herimite}. A greater value of \(j\)
will result in a curve with more turning points. It is expected that the
``U'' shape will be the easiest to detect, and as the shape gets more
...
Data collection period II was similar to Data collection I but focuses
on heteroskedasticity visual patterns formulated as
...
\noindent where \(\boldsymbol{y}\), \(\boldsymbol{x}\),
\(\boldsymbol{\varepsilon}\) are vectors of size \(n\).
...
\ref{eq:null-model}. For \(b \neq 0\), the variance-covariance matrix of
the error term \(\boldsymbol{\varepsilon}\) is correlated with the
predictor \(\boldsymbol{x}\), which will lead to the presence of
heteroskedasticity.

Visual patterns of heteroskedasticity are simulated using three
different shapes (\(a\) = -1, 0, 1) including ``left-triangle'',
...
the butterfly shape as the ratio parameter (\(b\)) is changed.

More details about the design of the data generating process are
...
predictor \(X_{raw}\), namely, uniform (\(U(-1, 1)\)), normal
(\(N(0, 0.3^2)\)), skewed (\(\text{lognormal}(0, 0.6^2)/3\)) and
discrete (\(U\{1, 5\}\)).
...
\textcolor{red}{Beyond 300, the scatterplot should probably be used with transparency or replaced with a density or binned plot as scatterplots suffer from over-plotting.}
...
additional parameter for the model polynomial order, \(j\), should be
incorporated. Intuitively, the large \(j\) means more complex pattern,
...
These \texttt{R} packages are used for the work: \texttt{tidyverse}
...
\texttt{rcartocolor} \citep{rcartocolor}. The study website is powered
by \texttt{PythonAnywhere} \citep{pythonanywhere} and the \texttt{Flask}
web framework \citep{flask}. The \texttt{jsPsych} framework
\citep{jspsych} is used to create behavioural experiments that run in
our study website.
