...
For the purposes of our calculations we have chosen to use an approach based on Kullback-Leibler divergence [@kullback1951information], coupled with simulation. This formulation defines effect size to be 
...
These `R` packages were used for the work: `tidyverse` [@tidyverse], `lmtest` [@lmtest], `mpoly` [@mpoly], `ggmosaic` [@ggmosaic], `kableExtra` [@kableextra], `patchwork` [@patchwork], `rcartocolor` [@rcartocolor]. The study website was powered by `PythonAnywhere` [@pythonanywhere] and `Flask` web framework [@flask]. The `jsPsych` framework [@jspsych] was used to create behavioural experiments that run in our study website. 
