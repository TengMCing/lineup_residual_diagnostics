...
For clarity and consistency, the analysis in Sections \ref{power-analysis}-\ref{hetero-analysis}, uses only on results from lineups generated with uniform fitted values, a total of `r unique(pull(filter(vi_survey, !attention_check, !null_lineup, x_dist == "uniform"), unique_lineup_id))` lineups and `nrow(filter(vi_survey, !attention_check, !null_lineup))` evaluations. More evaluations were made using the uniform fitted value distribution, and it avoids potentially confounding results of the analysis by combining values from different power curves. We also expect that visual patterns are more clearly visible when the fitted value distribution is uniform. The results in Section \ref{effect-of-fitted-value-distributions} examine the effect of fitted value distribution on power.  
...
```{r polypower, fig.height = 3, fig.width = 5, fig.cap = "Comparison of power between different tests for non-linear patterns (uniform fitted values only). The power curves are estimated using logistic regression, and the horizontal lines of dots represent human evaluations of lineups, with 0, 1 indicating non-detect and detect, respectively.  lines The visual test has multiple power curves estimated from bootstrap samples. The row of scatterplots plots at the bottom are examples of residual plots corresponding to the specific effect sizes marked by vertical lines in the main plot.", dev = 'png', dpi = 450}
