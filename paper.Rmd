...
\textcolor{red}{Scatterplots of residuals against the fitted values, and each of the explanatory variables, are commonly used to scrutinize their relationships}. If there are any visually discoverable associations, the model is potentially inadequate or incorrectly specified. \textcolor{red}{We can also potentially discover patterns not directly connected to a linear model assumption from these residual plots, such as the discreteness or skewness of the fitted values, and outliers.} To read residual plots, one looks for noticeable departures from the model such as non-linear pattern or heteroskedasticity. A non-linear pattern would suggest that the model needs to have some additional non-linear terms. Heteroskedasticity suggests that the error is dependent on the predictors, and hence violates the independence assumption. Statistical tests were developed to provide objective assessment, for example, of non-linear patterns [e.g. @ramsey1969tests], and heteroskedasticity  [e.g. @breusch1979simple]. 

<!--- ET: I suggest removing this to save space and we don't use this plot type.

\textcolor{red}{There are also other types of plots that can be used for assessing residuals. The normal probability plot is used to assess a normality assumption on the error distribution. Residuals are often plotted against  order of observations to check for collection bias, batch effects or autocorrelation. Partial residual plots assess the importance of an independent variable to a model, conditional on other variables, by plotting partial residuals against this variable. Partial residuals are computed from the difference between the response value and the fitted model excluding term of the independent variable of interest. A partial regression plot assesses the unique effect of an independent variable in a multiple regression model, by plotting residuals from the model with and without the this variable against each other.}
...
data.frame(j = c("2", "3", "6", "18", ""),
  ...
  knitr::kable(col.names = NULL,
               ...
               format = "latex",
               booktabs = TRUE, 
               caption  = 'Levels of the factors used in data collection periods I, II, and III.', 
...
\noindent where $\boldsymbol{y}$, $\boldsymbol{x}$, $\boldsymbol{\varepsilon}$, $\boldsymbol{x}_{raw}$, $\boldsymbol{z}_{raw}$ are vectors of size $n$, $\boldsymbol{1}_n$ is a vector of ones of size $n$, $He_{j}(.)$ is the $j$th-order probabilist's Hermite polynomials, $\varepsilon \sim N(\boldsymbol{0}_n, \sigma^2\boldsymbol{I}_n)$, and $g(\boldsymbol{x}, k)$ is a scaling function to enforce the support of the random vector to be $[-k, k]^n$ defined as
...
\noindent where $\displaystyle x_{min} = \min_{i\in \{1, ..., n\}} x_i$, $\displaystyle x_{max} = \max_{i\in \{1, ..., n\}} x_i$ and $x_i$ is the $i$-th entry of $\boldsymbol{x}$. 
...
\boldsymbol{x} &= g(\boldsymbol{x}_{raw}, 1),\\
\boldsymbol{\varepsilon} &\sim N(\boldsymbol{0}_n, \boldsymbol{1}_n + (2 - |a|)(\boldsymbol{x} - a\boldsymbol{1}_n)^\top(\boldsymbol{x} - a\boldsymbol{1}_n) b \boldsymbol{I}_n), 
\end{align*}
...
# \textcolor{red}{Limitations and practicality}
...
These `R` packages are used for the work: `tidyverse` [@tidyverse], `lmtest` [@lmtest], `mpoly` [@mpoly], `ggmosaic` [@ggmosaic], `kableExtra` [@kableextra], `patchwork` [@patchwork], `rcartocolor` [@rcartocolor]. The study website is powered by `PythonAnywhere` [@pythonanywhere] and the `Flask` web framework [@flask]. The `jsPsych` framework [@jspsych] is used to create behavioural experiments that run in our study website. 
