...
Figures \ref{fig:polypower} and \ref{fig:heterpower} present the power curves of various tests plotted against the effect size in the residuals. The effect size was computed at a 5% significance level and plotted on a natural logarithmic scale. To facilitate visual calibration of effect size values with the corresponding diagnostic plots, a sequence of example residual plots with increasing effect sizes is provided at the bottom of these figures. These plots serve as a visual aid to help readers understand how different effect size values translate to changes in the diagnostic plots.

displays the estimated power of a visual test, computed on evaluations of the lineups generated by the non-linearity model with uniform fitted values. Power is plotted against the natural logarithm of the effect size $log_e(E)$, computed at a 5% significance level. The small plots at the bottom of this figure shows a sequence of example residual plots of increasing effect size. These are for the readers to compare what a residual plot might look like at particular effect sizes. 

<!-- Where would you draw the line of too much non-linearity in the residuals? For the RESET test this is around log effect size 2, but for the visual test it is around 3.5.-->
...
```{r polypower, fig.height = 3, fig.width = 5, fig.cap = "Comparison of power between different tests for non-linear patterns (uniform fitted values only). The power curves are estimated using logistic regression, and the horizontal lines of dots represent decisions made by visual tests, with 0, 1 indicating non-reject and reject, respectively. The visual test has multiple power curves estimated from bootstrap samples. The row of scatterplots plots at the bottom are examples of residual plots corresponding to the specific effect sizes marked by vertical lines in the main plot.", dev = 'png', dpi = 450}
